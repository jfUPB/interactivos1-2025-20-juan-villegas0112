...
[Enlace a la aplicación a modificar](https://editor.p5js.org/generative-design/sketches/P_2_2_6_04)
...
[Enlace a la aplicación modificada](https://editor.p5js.org/juan-villegas0112/sketches/cqR05ryxO)
...
function updateButtonStates(newAState, newBState) {
  // Evento A PRESSED -> empezar un nuevo camino
  if (newAState === true && prevmicroBitAState === false) {
    newShape = new Shape(color(random(360), 80, 60, 50));
    newShape.addPos(microBitX, microBitY);
    print("A pressed -> nueva figura iniciada");
  }

  // Evento A RELEASED -> guardar el camino en shapes
  if (newAState === false && prevmicroBitAState === true) {
    if (newShape) {
      shapes.push(newShape);
      newShape = undefined;
      print("A released -> Shape guardada");
    }
  }

  //Evento B PRESSED -> aumentar longitud de lineas
  if (newBState === true && prevmicroBitBState === false) {
    lineLength += 2;
    print("B pressed -> lineLength aumento a " + lineLength);
  }
...
function draw() {
  ...
  switch (appState){
     case STATES.WAIT_MICROBIT_CONNECTION:
      ...
      // Dibujar formas almacenadas
      shapes.forEach(function(shape) {
        shape.draw();
        shape.update();
      });

      // Dibujar shape actual
      if (newShape) {
        newShape.addPos(microBitX, microBitY);
        newShape.draw();
        newShape.update();
...
[Video demostratativo](https://youtu.be/tlWgWGkSGHA?si=2FIf_iI12W81L0KO)
